...
def train():
    ...
    storage = Storage(obs_dim=go_env.obs_dim, action_dim=go_env.action_dim, max_timesteps=1024, gamma=0.98)
    rl_agent = PPO(env=go_env, actor_critic=actor_critic, storage=storage, device=device, lr=0.00025, action_scale=0.3, num_batches=1, num_epochs = 1)

    
    save_dir = f'checkpoints/{log_name}'
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    rl_agent.learn(save_dir, num_learning_iterations=50001, num_steps_per_val=250)
